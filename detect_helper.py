...
def getOrientation(pts, img):
    ...
    label = " " + str(-int(np.rad2deg(angle))) + " "
...
def get_edge_lines(image):
    ...
    #edge_image = PI.get_grid_image(image = image)

    # Convert image to binary
    edges = PI.get_canny(image = gray, threshold_low=50, threshold_high=200)
    lines = cv2.HoughLinesP(edges, 1, np.pi/180, threshold=30, maxLineGap=200)#PI.get_HoughLines(image=bw, )
    

    length1 = 0
    length2 = 0
    max_length_point1 = [0,0,0,0]
    max_length_point2 = [0,0,0,0]
    ...
    for line in lines:
        x1, y1, x2, y2 = line[0]
        if(y2 > y1):
            _length1 = math.sqrt(y2**2-y1**2+x2**2-x1**2)
            if(length1<_length1):
                length1 = _length1
                max_length_point1[0] = x1
                max_length_point1[1] = y1
                max_length_point1[2] = x2
                max_length_point1[3] = y2
        else:
            _length2 = math.sqrt(abs(y2**2-y1**2)+x2**2-x1**2)
            if(length2<_length2):
                length2 = _length2
                max_length_point2[0] = x1
                max_length_point2[1] = y1
                max_length_point2[2] = x2
                max_length_point2[3] = y2
            

    image = cv2.line(image, (max_length_point1[0], max_length_point1[1]), (max_length_point1[2], max_length_point1[3]), (0,0,255), 3)
    image = cv2.line(image, (max_length_point2[0], max_length_point2[1]), (max_length_point2[2], max_length_point2[3]), (0,0,255), 3)
...
def get_edge(image):
    ...
    bw = PI.get_binary_image(image=gray)
